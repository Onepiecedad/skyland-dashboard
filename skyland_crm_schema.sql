...
CREATE TABLE customers (
  ...
  source TEXT,
  -- 'website', 'telegram', 'referral', 'repeat'
  -- Anteckningar
  notes TEXT,
  -- Metadata
  tags TEXT [] DEFAULT '{}'
...
CREATE TABLE inbox (
  ...
  status TEXT DEFAULT 'pending' CHECK (
    status IN ('pending', 'processed', 'spam', 'error')
  ),
  error_message TEXT,
  -- Om status = 'error'
...
CREATE TABLE leads (
  ...
  source_id TEXT,
  -- t.ex. telegram chat_id eller email message_id
  inbox_id UUID REFERENCES inbox(id),
  -- Koppling till ursprunglig inbox
  ...
  ai_priority TEXT DEFAULT 'normal' CHECK (
    ai_priority IN ('urgent', 'high', 'normal', 'low')
  ),
  ai_summary TEXT,
...
ALTER TABLE inbox
ADD CONSTRAINT inbox_lead_fk FOREIGN KEY (lead_id) REFERENCES leads(id);
-- ============================================================
-- 5. JOBS - Arbetsorder
-- ============================================================
CREATE TABLE jobs (
...
CREATE TABLE job_items (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  -- Typ
  item_type TEXT DEFAULT 'labor' CHECK (
    item_type IN ('labor', 'part', 'material', 'other')
  ),
  -- Beskrivning
...
CREATE TABLE activity_log (
  ...
  SET NULL,
    inbox_id UUID REFERENCES inbox(id) ON DELETE
  SET NULL,
    -- Metadata
    actor TEXT NOT NULL DEFAULT 'system',
    -- 'system', 'n8n', 'user', 'ai', 'telegram'
    actor_id TEXT,
    -- user_id eller bot_id
    -- Extra data
    metadata JSONB,
    -- För snabb filtrering
    entity_type TEXT,
    -- 'customer', 'lead', 'job', 'inbox'
    entity_id UUID
);
...
CREATE TABLE messages (
  ...
  channel TEXT NOT NULL CHECK (
    channel IN ('email', 'telegram', 'sms', 'phone', 'website')
  ),
  -- Innehåll
  subject TEXT,
  content TEXT,
  ...
  -- Referenser
  external_id TEXT,
  -- telegram message_id, email id
  thread_id TEXT,
  -- för att gruppera konversationer
...
VALUES (
  ...
  (
    'ai_enabled',
    'true',
    'AI-klassificering aktiverad'
  ),
  (
    'ai_model',
    '"llama3.1-8b"',
    'AI-modell för klassificering'
  ),
  (
    'telegram_notifications',
    'true',
    'Skicka notiser via Telegram'
  ),
  (
    'auto_respond_enabled',
    'false',
    'Automatiska svar aktiverat'
  );
-- ============================================================
-- 10. INDEXES
-- ============================================================
...
-- Settings updated_at
CREATE OR REPLACE FUNCTION update_settings_timestamp() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER settings_updated_at BEFORE
UPDATE ON settings FOR EACH ROW EXECUTE FUNCTION update_settings_timestamp();
-- ============================================================
...
CREATE POLICY "Authenticated read" ON customers FOR
SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated insert" ON customers FOR
INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Authenticated update" ON customers FOR
UPDATE USING (auth.role() = 'authenticated');
-- ============================================================
-- BOATS POLICIES
-- ============================================================
CREATE POLICY "Service role full access" ON boats FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Authenticated read" ON boats FOR
SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated insert" ON boats FOR
INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Authenticated update" ON boats FOR
UPDATE USING (auth.role() = 'authenticated');
-- ============================================================
-- INBOX POLICIES
-- ============================================================
CREATE POLICY "Service role full access" ON inbox FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Authenticated read" ON inbox FOR
SELECT USING (auth.role() = 'authenticated');
-- ============================================================
-- LEADS POLICIES
-- ============================================================
CREATE POLICY "Service role full access" ON leads FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Authenticated read" ON leads FOR
SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated insert" ON leads FOR
INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Authenticated update" ON leads FOR
UPDATE USING (auth.role() = 'authenticated');
-- ============================================================
-- JOBS POLICIES
-- ============================================================
CREATE POLICY "Service role full access" ON jobs FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Authenticated read" ON jobs FOR
SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated insert" ON jobs FOR
INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Authenticated update" ON jobs FOR
UPDATE USING (auth.role() = 'authenticated');
-- ============================================================
-- JOB_ITEMS POLICIES
-- ============================================================
CREATE POLICY "Service role full access" ON job_items FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Authenticated read" ON job_items FOR
SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated insert" ON job_items FOR
INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Authenticated update" ON job_items FOR
UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated delete" ON job_items FOR DELETE USING (auth.role() = 'authenticated');
-- ============================================================
-- ACTIVITY_LOG POLICIES
-- ============================================================
CREATE POLICY "Service role full access" ON activity_log FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Authenticated read" ON activity_log FOR
SELECT USING (auth.role() = 'authenticated');
-- Activity log är append-only för användare
CREATE POLICY "Authenticated insert" ON activity_log FOR
INSERT WITH CHECK (auth.role() = 'authenticated');
-- ============================================================
-- MESSAGES POLICIES
-- ============================================================
CREATE POLICY "Service role full access" ON messages FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Authenticated read" ON messages FOR
SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated insert" ON messages FOR
INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Authenticated update" ON messages FOR
UPDATE USING (auth.role() = 'authenticated');
-- ============================================================
-- SETTINGS POLICIES
-- ============================================================
CREATE POLICY "Service role full access" ON settings FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Authenticated read" ON settings FOR
SELECT USING (auth.role() = 'authenticated');
...
CREATE OR REPLACE FUNCTION get_setting(p_key TEXT) RETURNS JSONB AS $$ BEGIN RETURN (
    SELECT value
    FROM settings
    WHERE key = p_key
  );
...
CREATE OR REPLACE FUNCTION set_setting(p_key TEXT, p_value JSONB) RETURNS VOID AS $$ BEGIN
